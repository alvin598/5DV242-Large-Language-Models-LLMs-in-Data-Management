...
# setup paths to corpora and vocab files
...
vocab_size = 100
corpus_file = PATH / "corpora" / "stories.txt"
...
if __name__ == '__main__':
    ...
    if usage == 'inference':
        ...
        with torch.no_grad():
            while True:
                prompt = input("prompt:") + " "
                x = torch.tensor(train_dataset.encode(prompt), dtype=torch.long)[None, ...].to('mps' if torch.backends.mps.is_available() else 'cpu')
        ...
        corpus_file =  PATH / "corpora" / "stories-sft.txt"
        #corpus_file = PATH / "corpora" / "reasoning-sft.txt"
        prompt_file = corpus_file.with_name(corpus_file.stem + "-prompts.txt")
